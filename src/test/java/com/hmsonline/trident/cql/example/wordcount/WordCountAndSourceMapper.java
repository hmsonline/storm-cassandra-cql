...
public class WordCountAndSourceMapper implements CqlRowMapper<List<String>, Number>, Serializable {
	...
	// LoggerFactory.getLogger(WordCountAndSourceMapper.class);
	...
	public Statement retrieve(List<String> keys) {
		...
		Select statement = QueryBuilder.select().column(SOURCE_KEY_NAME).column(WORD_KEY_NAME).column(VALUE_NAME)
